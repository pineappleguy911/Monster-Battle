...
document.addEventListener('keydown', (event) => {
    switch (event.key) {
      ...
      case 'w':
        ...
        case 'o':
        shootAt(player.X, player.Y + 1)
        shootAt(player.X + 1, player.Y + 1)
        shootAt(player.X - 1, player.Y + 1)
        shootAt(player.X - 2, player.Y)
        shootAt(player.X + 2, player.Y)
        shootAt(player.X + 1, player.Y - 2)
        shootAt(player.X - 1, player.Y - 2)
        break;
...
function generateRandomBoard() {
    ...
    [playerY, playerX] = randomEmptyPosition(newBoard);
    ...
    for (let y = 0; y < BOARD_SIZE; y++) {
        for (let x = 0; x < BOARD_SIZE; x++) {
            ...
            if (getCell(board, x, y) === 'W') {
                cell.classList.add('wall'); // ww =ei tänne >:c
            }
            

            else if (getCell(board, x, y) === 'P') {
                cell.classList.add('player'); // p  = sinä vautsi vauu
            }

            else if (getCell(board, x, y) === 'H') {
                cell.classList.add('ghost'); // h= kummitus  pelotava
            }

            
            else if (getCell(board, x, y) === 'B') {
                cell.classList.add('bullet'); // b = ammus pew pew
                setTimeout(() => {
                    setCell(board, x, y, ' ')
                    drawBoard(board);
                }, 500);
